...
class subCats extends TextLayer
	...
	@define 'active',
		...
		set: (value) ->
			...
			if @options.active is true
				@fontFamily = subnavON.fontFamily
				@color = subnavON.color
			else
				@fontFamily = subnav.fontFamily
				@color = subnav.color
...
for subcat, i in SubCats_Breakfast
	subcat = new subCats
		...
		style:
			position: "relative"
	SubCats.push(subcat)
	subNavBar_container.width = (subcat.width+40)*i
...
subCatLine.midX = SubCats[0].midX + subNavBar_container.x-30
...
print SubCats[2].midX 
print subNavBar_container.x
SubCats[2].onClick ->
	subCatLine.animate
		midX: Utils.round(SubCats[2].midX,0) +subNavBar_container.x
		options:
			time: 1
	SubCats[2].active = true
	SubCats[0].active = false
